...
- (IBAction)headTapped {
    ...
    //
    UIAlertAction *photoAction = [UIAlertAction actionWithTitle:@"从相册选择" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
...
- (IBAction)saveButtonPressed {
    ...
    if (_headImage != nil) {
        ...
        [helper uploadWithURLString:urlString data:data success:^(NSURLResponse *response, id object) {
            ...
            if (photo != nil) {
                NSString *urlString = [kURLTypeFileUploadPrefix stringByAppendingString:photo];
...
- (void)commitWithPhotoURLString:(NSString *)urlString {
    ...
    [helper sendRequestWithURLString:kURLTypePublish parameters:params success:^(NSURLSessionDataTask *task, NSDictionary *response) {
        ...
        if (code == 200) {
            ...
            [UIHelper showErrorWithText:response[@"mag"] inView:self.view];
        }
